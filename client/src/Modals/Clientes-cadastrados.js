...
export default function ClientesCadastrados({
  ...
  const saveEdit = (id_cliente) => {
    ...
    if (!nome || !cpfOuCnpj) {
      ...
      fetch(
        `http://localhost:5000/api/crm/clientsRegistered/${id_cliente}/edit`,
        {
          method: "PUT",
          headers: {
  ...
  const updateToInactive = (id_cliente) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(
          `http://localhost:5000/api/crm/clientsRegistered/${id_cliente}/delete`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
                ...
                clientes.map((cliente, index) => (
                  <tr key={index}>
                    ...
                    {editandoId === cliente.id_cliente ? (
                      <>
                        <td>
                          <button onClick={() => saveEdit(cliente.id_cliente)}>
                            Salvar
                          </button>
                        </td>
                        <td>
