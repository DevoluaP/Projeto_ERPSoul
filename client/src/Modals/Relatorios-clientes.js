...
export default function RelatoriosClientes({
  isOpenRelatoriosClientes,
  setCloseModal,
}) {
  const [clientes, setClientes] = useState([]);
  const formatCpfCnpj = (cpfOuCnpj) => {
    return mask(cpfOuCnpj);
  };
  ...
  const handleSubmit = (e) => {
    ...
    const { nome, cpfOuCnpj, dt_nasc, id_natureza } = formData;

    if (!nome && !cpfOuCnpj && !dt_nasc && !id_natureza) {
      Swal.fire({
      ...
      (cpfOuCnpj.length === 14 &&
        id_natureza.length > 0 &&
        id_natureza !== "2") ||
      id_natureza.length > 2
    ) {
      Swal.fire({
  ...
  const exportToExcel = (clientes) => {
    ...
    clientes.forEach((cliente) => {
      const clienteData = [
        ...
        cliente.id_natureza === 1 ? "Físico" : "Jurídico",
      ];
  ...
  const exportToText = (clientes) => {
    ...
    clientes.forEach((cliente) => {
      texto += `${cliente.id_cliente} | ${cliente.nome} | ${formatCpfCnpj(cliente.cpf || cliente.cnpj)} | ${new Date(cliente.dt_nasc).toLocaleDateString("pt-BR")} | ${cliente.endereco_completo || ""} | ${cliente.id_natureza === 1 ? "Físico" : "Jurídico"}\n`;
    });
