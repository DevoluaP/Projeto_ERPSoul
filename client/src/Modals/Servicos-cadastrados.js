...
export default function ServicosCadastrados({
  ...
  const startEdit = (service) => {
    ...
    setEditandoDados({
      ...
      data_vencimento: new Date(service.dt_vencimento)
        .toISOString()
        .split("T")[0],
  ...
  const saveEdit = (id_servico) => {
    ...
    if (
      ...
      fetch(
        `http://localhost:5000/api/services/servicesRegistered/${id_servico}/edit`,
        {
          method: "PUT",
          headers: {
  ...
  const updateToInactive = (id_servico) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(
          `http://localhost:5000/api/services/servicesRegistered/${id_servico}/delete`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
          ...
          .then((data) => {
            if (data.mensagem === "Serviço excluído com sucesso!") {
              ...
              setServices(
                services.filter((service) => service.id_servico !== id_servico),
              );
            } else {
              Swal.fire({
                title: "Erro!",
                ...
                services.map((service, index) => (
                  <tr key={index}>
                    ...
                    {editandoId === service.id_servico ? (
                      ...
                      <>
                        <td>
                          {parseFloat(service.valor_servico).toLocaleString(
                            "pt-BR",
                        ...
                        <td>
                          {new Date(service.dt_vencimento).toLocaleDateString(
                            "pt-BR",
                          )}
                        </td>
                        <td>
                    ...
                    {editandoId === service.id_servico ? (
                      ...
                      <>
                        <td>
                          <i className="fa-solid fa-pen" onClick={() => startEdit(service)} />
                        </td>
                        <td>
                          <i className="fa-solid fa-trash" onClick={() => updateToInactive(service.id_servico)} />
                        </td>
                      </>
                    )}
                  </tr>
