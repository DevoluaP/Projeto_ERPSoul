...
exports.registerClient = async (req, res) => {
  ...
  if (cpfOuCnpj.length === 14 && id_natureza !== "2") {
...
exports.editClient = async (req, res) => {
  ...
  if (!nome || !cpfOuCnpj) {
    return res
      .status(400)
      .json({ mensagem: "Os campos não podem ficar vazio!" });
  }
...
exports.reportsClients = async (req, res) => {
  const id_usuario = req.user.userId;
  const { nome, cpfOuCnpj, dt_nasc, id_natureza } = req.body;
  ...
  if ((cpfOuCnpj.length === 14 && id_natureza.length > 0 && id_natureza !== "2") || id_natureza.length > 2) {
    return res.status(400).json({ mensagem: "Tipo de cliente incorreto!" });
  }

  try {
    const [results] = await CrmModel.reports(
      id_usuario,
      nome,
      cpfOuCnpj,
      dt_nasc,
