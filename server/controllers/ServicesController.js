const ServicesModel = require("../models/ServicesModel.js");
...
exports.registerService = async (req, res) => {
  ...
  try {
    const client = await ServicesModel.verifyClient(
      id_usuario,
      id_cliente,
      cpfOuCnpj,
    );
...
exports.getServices = async (req, res) => {
  ...
  try {
    const [services] = await ServicesModel.findByUserId(id_usuario);

    const servicesWithClientName = await Promise.all(
      services.map(async (service) => {
        const clientName = await ServicesModel.getClientName(
          service.id_cliente,
          id_usuario,
        );
        return {
...
exports.editService = async (req, res) => {
  ...
  const { val_servico, imposto, id_natureza, data_vencimento, status_servico } =
    req.body;
  ...
  if (
    ...
    return res
      .status(400)
      .json({ mensagem: "Os campos não podem ficar vazios!" });
  }
  ...
  const valServico = parseFloat(
    valServicoStr.replace(".", "").replace(",", "."),
  );
  ...
  try {
    const [result] = await ServicesModel.edit(
      valServico,
      valImposto,
      id_natureza,
      data_vencimento,
...
exports.deleteService = async (req, res) => {
  ...
  try {
    const [result] = await ServicesModel.delete(id_servico);
...
exports.reportsServices = async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ mensagem: "Preencha pelo menos um dos campos!" });
  }
  ...
  try {
    const [results] = await ServicesModel.reports(
      id_usuario,
      nome_cliente,
      cod_servico,
      servico,
...
exports.exportReportsServices = async (req, res) => {
  ...
  try {
    const [results] = await ServicesModel.exportReports(servicoIds);
